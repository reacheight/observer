...
pub struct PeriodicObserver {
    ...
    routine: Option<Rc<RefCell<dyn PeriodicObserverRoutine>>>,
...
impl PeriodicObserver {
    pub fn init(
        ...
        interval_seconds: u32,
        game_time: Rc<RefCell<GameTimeObserver>>,
        routine: Rc<RefCell<dyn PeriodicObserverRoutine>>,
    ) {
    ...
    fn on_tick_start(&mut self, ctx: &Context) -> ObserverResult {
        let routine = self
            .routine
            .as_ref()
            .context("Observer is not initialized")?;
        ...
        if let Result::Err(err) = routine.borrow_mut().on_iteration(ctx, curent_time) {
            eprintln!("{err}");
        }
...
pub trait PeriodicObserverRoutine {
    fn on_iteration(&mut self, ctx: &Context, current_time: GameTime) -> ObserverResult;
}
